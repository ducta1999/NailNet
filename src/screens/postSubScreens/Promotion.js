...
export default class Promotion extends Component {
  ...
  async ensureDataFetched() {
    ...
    var promotions = await dataService.get(
      `api/promotions/getall?email=${user.email}&sortby=createtime`,
    );
      ...
      <Content>
        {this.state.loading == false ? (
          <View style={{flex: 1, backgroundColor: '#1F2426'}}>
            {this.renderListItem(this.state.promotions.items)}
          </View>
        ) : (
          <View>
            <Spinner color="red" />
            ...
            <List style={styles.list}>
              {items.map((item, i) => (
                <ListItem
                  ...
                  key={i}
                  onPress={() =>
                    this.props.navigation.navigate('PromotionPostDetail', {
                      id: item.id,
