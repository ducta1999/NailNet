...
export default class Shop extends Component {
  ...
  async ensureDataFetched() {
    ...
    var products = await dataService.get(
      `api/products/getall?email=${user.email}&sortby=createtime`,
    );
  ...
  render() {
    const {loading, mainCategories, listLoading, products} = this.state;
        ...
        <Content>
          {loading == false ? (
            <View style={{flex: 1, backgroundColor: '#1F2426'}}>
              {this.renderListItem(products.items)}
            </View>
          ) : (
            <View>
              <Spinner color="red" />
