...
export default class Job extends Component {
  ...
  async ensureDataFetched() {
    ...
    var jobs = await dataService.get(
      `api/jobs/getall?email=${user.email}&sortby=createtime`,
    );
      ...
      <Content>
        {this.state.loading == false ? (
          <View style={{flex: 1, backgroundColor: '#1F2426'}}>
            {this.renderListItem(this.state.jobs.items)}
          </View>
        ) : (
          <View>
            <Spinner color="red" />
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: 'OK',
          onPress: async () => {
            var result = await dataService.remove(`api/jobs/delete/${item.id}`);
