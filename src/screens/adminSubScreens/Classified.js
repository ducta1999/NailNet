...
export default class Classified extends Component {
  ...
  async ensureDataFetched() {
    ...
    console.log(mainCategoriesInDB);
    this.setState({
      categories: mainCategoriesInDB,
      loading: false,
    });
  }
        ...
        },
        {
          ...
          onPress: async () => {
            var result = await dataService.remove(
              `api/classifiedcategories/delete/${item.id}`,
            );

            if (result.status == 200) {
              toastService.success('Delete classified category successfully!');
              this.ensureDataFetched();
            } else {
