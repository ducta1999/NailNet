...
export default class Shop extends Component {
      ...
      var products =
        items[0].children.length == 0
          ...
          : await dataService.get(
              'api/products/getall?isApproved=false&productcategoryid=' +
          ...
          <List>
            {items.map((item, i) => (
              <ListItem
                ...
                noBorder
                onPress={() =>
                  this.props.navigation.navigate('ShopConfigDetail', {
                    id: item.id,
  ...
  async approve(item) {
    ...
    if (result.status == 200) {
      toastService.success('Approve product successfully!');
      this.ensureDataFetched();
    } else {
